...
// @version      2.2
// @description  将个人题库题目与答案导出为离线HTML/ZIP，内置ZIP生成（无需JSZip/CDN），答案图片支持油猴跨域获取回退，支持图片/公式嵌入、智能命名、去重，并发/延迟/封禁保护
// @author       Frank-678
...
(async function () {
  ...
  async function browserFetchBlob(url){
    const resp = await fetch(url, {method:'GET', mode:'cors', credentials:'include', cache:'force-cache', referrer: location.href});
  ...
  async function fetchBlobWithRetry(url, prefs){
    ...
    while(attempt<=maxRetries){
      try {
        const info = preferGM ? await gmFetchBlob(url) : await browserFetchBlob(url);
        if(info && (info.status === 429 || info.status === 403)) throw new Error('HTTP ' + info.status);
        return info;
      } catch(err){
        lastErr = err;
        if(!preferGM && isAnswerImageUrl(url)){
          try { return await gmFetchBlob(url); } catch(gmErr){ lastErr = gmErr; }
        }
